...
from pyhip import cudaPerf, jit, JIT, calc_diff
import torch

from pyhip.contrib.jit_gemm_splitk import gemm_splitk_wd
...
def _run_batch(kernel_type, M=1, weight_type=torch.bfloat16, TILE_M=16, TILE_N=32, run_count=10, N=4096, K=4096):
    ...
    if weight_type == torch.float4_e2m1fn_x2:
        ...
        def weight_per_128x128_quant(weight, quant_dtype):
            ...
            weight_qt = weight_qt.view(N_dim, K_dim)
        ...
        assert 0, f'Only fp4/fp8 weight is supported in this test, current weight_type={weight_type}'
...
def test_acc(TILE_M=32, TILE_N=64, N=4096, K=4096):
    ...
    entry_common('mxn_splitk_2s', M=M, prec=[get_fp4type_if_valid(), get_fp8type()], TILE_M=TILE_M, TILE_N=TILE_N, N=N, K=K, run_count=0)
    print("ok")
...
def test_perf(M, TILE_M=32, TILE_N=64, N=4096, K=4096):
    init_env()
    perf = {}
    perf.update(entry_common('mxn_splitk_2s', M=M, prec=[get_fp4type_if_valid(), get_fp8type()], TILE_M=TILE_M, TILE_N=TILE_N, N=N, K=K))
    return perf
