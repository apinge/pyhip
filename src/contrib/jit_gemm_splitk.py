...
def gemm_splitk_wd(J:JIT,
                   ...
                   p_w_scale:"float*",
                   M:"int",):
    assert weight_dtype == torch.float4_e2m1fn_x2 or weight_dtype == torch.float8_e4m3fn or weight_dtype == torch.float8_e4m3fnuz or weight_dtype == torch.bfloat16
    num_split_k = 4
    ...
    if weight_dtype == torch.float4_e2m1fn_x2:
        ...
        voffset_a[0] = J.gpr((lane_mod_16 + J.blockIdx.y * BLOCK_TILE_SIZE_M) * stride_A) + (J.threadIdx.x // 16) * (a_element_num_per_thread * J.sizeof_bf16)
