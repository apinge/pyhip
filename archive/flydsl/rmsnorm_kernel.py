...
def build_rmsnorm_fusedadd_module(M: int, N: int, dtype_str: str):
    arch = get_hip_arch()
    USE_HW_CVT_PK_BF16_F32 = (arch == "gfx950") or str(arch).startswith("gfx95")
    ...
    def rmsnorm_fusedadd_kernel(
        ...
        if N >= tile_cols and N % tile_cols == 0 and elem_bits <= 16:
            ...
            out_rsrc = buffer_ops.create_buffer_resource(Output, max_size=True)
            gamma_rsrc = buffer_ops.create_buffer_resource(Gamma, max_size=True)
            residual_rsrc = buffer_ops.create_buffer_resource(Residual, max_size=True)
            ...
            def _store_vec(data, rsrc, col_byte_off, soff=None):
                ...
                buffer_ops.buffer_store(data, rsrc, dw, soffset_bytes=soff)
            # 将fp32转换为bf16的向量
            def _turn_fp32_to_bf16_vec(data):
                if USE_HW_CVT_PK_BF16_F32:
                    out_e = data.truncf(vec_type_e)
                else:
                    vec_i32_ty = T.vec(VEC_WIDTH, T.i32)
                    vec4_i32_ty = T.vec(VEC_WIDTH // 2, T.i32)
                    vec_bf16_ty = T.vec(VEC_WIDTH, elem_type)
            ...
            for tile_i in range_constexpr(num_tiles):
                col_bytes = ArithValue(thr_col_bytes) + (tile_i * tile_cols * elem_bytes)
                vec_e = _load_vec(in_rsrc, col_bytes, soff=row_soffset)
                res_e = _load_vec(residual_rsrc, col_bytes, soff=row_soffset) # residaual和input同型
                if cache_as_elem:
                    r = res_e.extf(vec_type_c)
                    x = vec_e.extf(vec_type_c)
                    x_av = ArithValue(r) + ArithValue(x)
                    # 只缓存 x+residual 的 f32 向量；pass2 归一化的是该值，且不可再 extf（f32→f32 非法）
                    fused_local.append(x_av)
                    # bf16 可用软件 pk 舍入；f16 必须用 truncf，勿走 _turn_fp32_to_bf16_vec（其为 upper-16bit bf16 逻辑）
                    if dtype_str == "bf16":
                        x_av_tmp = _turn_fp32_to_bf16_vec(x_av)
                    else:
                        x_av_tmp = x_av.truncf(vec_type_e)
                    x_av_out = vector.bitcast(i32_vec_ty, x_av_tmp)
                    _store_vec(x_av_out, residual_rsrc, col_bytes, soff=row_soffset)
                else:
                    x = vec_e
                    r = res_e
                    x_av = ArithValue(r) + ArithValue(x) 
                    ...
                    x_av_out = vector.bitcast(i32_vec_ty, x_av)
                    _store_vec(x_av_out, residual_rsrc, col_bytes, soff=row_soffset)
            ...
            # Pass 2: normalize + gamma + store（fused 路径缓存的是已在 f32 的 x+residual）
            for tile_i in range_constexpr(num_tiles):
                col_bytes = ArithValue(thr_col_bytes) + (tile_i * tile_cols * elem_bytes)
                ...
                x_fused_tile = fused_local[tile_i]
                x_av = ArithValue(x_fused_tile)
                g_av = ArithValue(g)
                y = (x_av * rrms_splat_av) * g_av
                y_val = y
                ...
                if dtype_str == "bf16":
                    ...
                    #     packed = even | odd_sh
                    #     out_e = vector.bitcast(vec_bf16_ty, packed)
                    out_e = _turn_fp32_to_bf16_vec(y_val)
                elif dtype_str == "f32":
                    out_e = y_val
                else:
                    out_e = y_val.truncf(vec_type_e)
            ...
            # ==============================================================
            # Generic path: scalar 2-pass for arbitrary N（含 f32：fused 向量快路径因 256-bit buffer 存取在后端受限未启用）
            # ==============================================================
            from flydsl.expr.arith import ArithValue
            ...
            for base_idx_int in range_constexpr(0, N, BLOCK_THREADS):
                ...
                idx_safe = is_valid.select(idx, c0_i)
                x_e = _load_scalar(row_div, idx_safe)
                r_e = _load_scalar(res_div, idx_safe)
                x = x_e if dtype_str == "f32" else x_e.extf(compute_type)
                r = r_e if dtype_str == "f32" else r_e.extf(compute_type)
                x_f = ArithValue(x) + ArithValue(r)
                x2 = x_f * x_f
                x2_safe = is_valid.select(x2, c_zero_f)
                thread_sumsq = ArithValue(thread_sumsq) + x2_safe
                if arith.cmpi(arith.CmpIPredicate.ult, idx, c_N_i32):
    ...
    def launch_rmsnorm_fusedadd(
        ...
        idx_m = arith.index_cast(T.index, m_in)
        launcher = rmsnorm_fusedadd_kernel(Input, Gamma, Residual, Output)
        launcher.launch(
            grid=(idx_m, 1, 1),
            block=(BLOCK_THREADS, 1, 1),
            stream=stream,
    ...
    return launch_rmsnorm_fusedadd