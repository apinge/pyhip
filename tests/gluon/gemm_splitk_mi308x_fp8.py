...
def _run_batch(kernel_type, M=1, weight_type=torch.bfloat16, TILE_M=16, TILE_N=32, run_count=10, N=4096, K=4096):
    ...
    if weight_type == torch.float4_e2m1fn_x2:
        ...
        # gemm_splitk_kernel 仅识别 float8_e4m3fn；需 SHUFFLE=1；scale 块为 128x128
        assert N % 128 == 0 and K % 128 == 0, f"fp8 scale grid needs N,K divisible by 128, got N={N}, K={K}"
        ...
        raise AssertionError(
            f"unsupported weight_type={weight_type} (fp8 请用 torch.float8_e4m3fn；get_fp8type() 在非 gfx950 上可能是 e4m3fnuz，当前 kernel 未支持)"
        )
...
def test_perf(M, TILE_M=32, TILE_N=64, N=4096, K=4096, prec=(torch.bfloat16,)):
    init_env()
    perf = {}
    # prec: 权重 dtype，例如 (torch.bfloat16,) 或 (torch.float8_e4m3fn,)（fp8 需 SHUFFLE=1，见 gemm_splitk Args）
    perf.update(entry_common("mxn_splitk_2s", M=M, prec=list(prec), TILE_M=TILE_M, TILE_N=TILE_N, N=N, K=K))
    return perf
...
if __name__ == '__main__':
    ...
    perf = {}
    dict_tile_mn = {}
    # 默认 bf16：在 gfx942 上可完整编译运行。fp8 路径在部分 Triton/LLVM 上仍可能在 tt.dot 合法化阶段失败。
    # 需要测 fp8 时：  GEMM_SPLITK_FP8=1 python gemm_splitk_mi308x_fp8.py
    if os.environ.get("GEMM_SPLITK_FP8", "0") == "1":
        weight_prec = (torch.float8_e4m3fn,)
        print("GEMM_SPLITK_FP8=1: 使用 fp8 权重（若编译失败，需换 gfx950/CDNA4 或更新 Triton）")
    else:
        weight_prec = (torch.bfloat16,)
    ...
    for M in Ms:
        ...
        perf = merge(
            perf,
            test_perf([M], TILE_M=TILE_M, TILE_N=TILE_N, N=N, K=K, prec=weight_prec),
        )
