...
HK = 4
S = 256
KV_LEN = 60318 # 我们的实际例子是8k 16K为主  45694是长上下文情况 保留
#KV_LEN = 512
DT = torch.bfloat16
BLOCK_SIZE = 1
...
def test_aiter(query,
               ...
               fp8_out_scale=None,
               partition_size=KV_PART_SIZE, # 注意这里已经是集成了pa的aiter 
               mtp=1,):
...
if 1:
    ...
    aiter_out = test_aiter(query=query, key_cache=key_caches[-1], value_cache=value_caches[-1], scale=scale, 
    ...
    if ref_out is not None:
        if not torch.allclose(aiter_out, ref_out, rtol=0.01, atol=0.01): #, "aiter acc is wrong"
            # breakpoint()
            print('aiter acc wrong')
        else:
            print('aiter acc ok')
    i = 0
    if 0:
...
def div_up(x, y):
    return (x + y - 1) // y
# -g -ggdb -O1
hip = pyhip.module("pa-256.cpp", f"-D{HQ=} -D{HK=} -D{S=} -D{BLOCK_SIZE=} -DSCALE={scale} -D{KV_PART_SIZE=} -D{FAKE_Q=} -D{FAKE_K_IDX=} -D{OUTPUT_QK=}")
pa = hip.pa
...
# check q*k
if 1:
    # ref 是 [B, HK, （HQ // HK）, S] @ [B, HK, S, KV_LEN] -> [B, HK, （HQ // HK）, KV_LEN]
    # ref_qk = query.reshape(B, HK, HQ // HK, -1) @ key_caches[-1][1:KV_LEN*B +1].reshape(B, -1, HK, S).permute(0, 2, 3, 1)
    # ref_qk = ref_qk.to(torch.float32)
    # ref_qk = ref_qk * scale
    # if OUTPUT_QK:
    ...
    if len(idx[0]):
        print(f'idx = {idx}\nref_out={ref_out[idx]}\ncur={cur_out[idx]}')
    # 维持原版的 check方式 不要动
    #assert torch.allclose(out, ref_out, rtol=0.01, atol=0.01), "aiter is wrong compare with aiter"
    assert torch.allclose(ref_out, my_out, rtol=0.01, atol=0.01), "pa out is wrong"
    #breakpoint()
