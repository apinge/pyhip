...
/* Qwen3-Omni text path style (32 Q heads / 4 KV, head_dim 128); matches /root/workspace/pa pa_ragged.cpp.jinja defaults. */
#ifndef HQ
#define HQ 32
#define HK 4
#define S 128
...
__global__ void __launch_bounds__(NUM_THREADS, 2) pa(
    ...
    for (uint part_idx = 0; part_idx < KV_PART_SIZE / KV_MIN_PART_SIZE; part_idx++) {
        ...
        if (BLOCK_SIZE == 1) {
            for (uint n = 0; n < KV_PART_SIZE_WARP / 4; n++) {
                ...
                k_idxs[n] = (global_row_id < kv_len_end) ? idx_lds[n * 4 + key_load_row_id] : 0;
    #endif
            }
        }
        // key -> reg
    ...
    #else
                k_idxs[n] = (global_row_id < kv_len_end)
                                ? buffer_load_dword<uint>(idx_buf, 0, (key_load_row_id + cur_kv_len_start) * sizeof(uint), n * 4 * sizeof(uint))
                                : 0;
    ...
    float real_max[(HQ / HK + 3) / 4], real_sum[(HQ / HK + 3) / 4];
    for (uint i = 0; i < (HQ / HK + 3) / 4; i++) {
        uint m_token_id = (HQ / HK + 3) / 4 * warp_id + i;
