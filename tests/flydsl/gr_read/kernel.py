...
class Config:
    ...
    def validate(self):
        ...
        if self.down_mode == "wave_splitk":
            if self.down_waves not in (2, 4, 8) or K % (self.down_waves * self.down_block_k):
                raise ValueError("wave split-K requires 2/4/8 waves evenly dividing K tiles")
            ...
            if self.down_copy_bits not in (64, 128) or self.down_block_k % 32:
                raise ValueError("wave split-K copy requires 64/128 bits and K tiles divisible by 32")
...
def _launchers(rows: int, dtype: torch.dtype, config: Config):
    ...
    @fx.struct
    class DownShared:
        partials: fx.Array[fx.Float32, bm * dn * down_waves, 16]

    @flyc.kernel
    def down_wave_splitk_silu(X: fx.Tensor, W: fx.Tensor, A: fx.Tensor):
        tid = fx.thread_idx.x
        ...
        x_tile = fx.flat_divide(x, fx.make_tile(bm, down_block_k))[None, None, im, None]
        w_tile = fx.flat_divide(w, fx.make_tile(dn, down_block_k))[None, None, jn, None]
        if fx.const_expr(down_flip):
            a_tile, b_tile = w_tile, x_tile
            c_tile = fx.select(partials[None, None, wave], [1, 0])
        else:
            a_tile, b_tile = x_tile, w_tile
        ...
        copy_ab = fx.make_copy_atom(
            fx.rocdl.BufferCopy128b() if down_copy_bits == 128 else fx.rocdl.BufferCopy64b(), elem
        )
        ...
        # The colleague's split-K pattern: each wave owns a K slice, then LDS reduction.
        for ki, state in range(fx.Index(0), fx.Index(K // down_waves // down_block_k), fx.Index(1), init=[fc.load()]):
            fc.store(state[0])
            if fx.const_expr(down_interleave):
                kt = fx.Int32(ki) * down_waves + wave
            else:
                kt = wave * (K // down_waves // down_block_k) + fx.Int32(ki)
        ...
        fx.gpu.barrier()
        out = fx.make_view(fx.get_iter(A), fx.make_layout((padded_rows, R), (R, 1)))
        for i in range_constexpr((bm * dn + down_waves * 64 - 1) // (down_waves * 64)):
            index = tid + i * down_waves * 64
            if index < bm * dn:
                ...
                fx.memref_store(activated, out, (im * bm + row, jn * dn + col))
...
class GRRead:
    ...
    def __init__(self, rows, w_down, w_up, config=None):
        ...
        intermediate_splits = 1 if config.down_mode == "wave_splitk" else config.split_k
        self.partial = torch.empty((intermediate_splits * padded * R,), dtype=torch.float32, device=self.device)
