...
if __package__:
    from . import prefetch_up as control
    from .combined_host import CombinedPaddedGRRead, _check_input
else:
    import prefetch_up as control
    from combined_host import CombinedPaddedGRRead, _check_input
...
class DownConfig:
    ...
    def validate(self):
        ...
        if self.prefetch_unroll == 2 and (not self.prefetch or K // self.waves // self.block_k // self.global_split % 2 != 1):
...
def default_config(rows):
    ...
    return DownConfig(block_m=32, block_k=128, global_split=4, prefetch_unroll=2 if rows <= 25 else 1)
...
def down_launcher(rows, config):
    ...
    def down_wave_splitk_pipeline(X: fx.Tensor, W: fx.Tensor, A: fx.Tensor):
        ...
        x = fx.rocdl.make_buffer_tensor(
            fx.make_view(fx.get_iter(X), fx.make_layout((rows, K), (K, 1))), max_size=False
        )
        ...
        tiled = fx.make_tiled_mma(
            mma, fx.make_layout((1, 1, 1), (0, 0, 0)),
        ...
        if fx.const_expr(prefetch):
            ...
            for ki, state in range(
                fx.Index(1), fx.Index(iterations), fx.Index(prefetch_unroll), init=[fa.load(), fb.load(), fc.load()]
            ):
                fa.store(state[0])
                fb.store(state[1])
                fc.store(state[2])
            ...
            for ki, state in range(fx.Index(0), fx.Index(iterations), fx.Index(1), init=[fc.load()]):
                fc.store(state[0])
                if fx.const_expr(interleave):
                    kt = sk * (K // split // bk) + fx.Int32(ki) * waves + wave
                else:
        ...
        for i in range_constexpr((bm * dn + waves * 64 - 1) // (waves * 64)):
            ...
            if index < bm * dn:
                ...
                if fx.const_expr(split == 1):
                    z = total * 0.25
                    exponent = fx.Float32(fx.rocdl.exp2(fx.T.f32, fx.arith.unwrap(-z * LOG2E)))
                    inverse = fx.Float32(fx.rocdl.rcp(fx.T.f32, fx.arith.unwrap(1.0 + exponent)))
                    fx.memref_store(z * inverse, out, (im * bm + row, jn * dn + col, sk))
                else:
    ...
    def launch(X: fx.Tensor, W: fx.Tensor, A: fx.Tensor, stream: fx.Stream):
        down_wave_splitk_pipeline(X, W, A).launch(
            grid=(padded_rows // bm, R // dn, split), block=(waves * 64, 1, 1), stream=stream
        )
...
def pair_launcher(rows, down_config, up_config):
    ...
    _, up = control._launchers(rows, torch.bfloat16, up_config)
...
class LargeDownGRRead:
    """A/B candidate sharing the control's weights, but not its output/workspace."""
    ...
    def __init__(self, prepared, config=None):
        ...
        if not 17 <= prepared.rows <= 32:
            raise ValueError("experimental large down is restricted to T=17..32")
        config = default_config(prepared.rows) if config is None else config
        config.validate()
        expected = control.default_config(prepared.rows)
        if not prepared.config.preshuffle or prepared.config.down_mode != expected.down_mode:
            raise ValueError("requires the original preshuffled large-T control")
        self.rows, self.dtype, self.device = prepared.rows, prepared.dtype, prepared.device
        self.config, self.up_config = config, prepared.config
        self.w_down, self.w_up = prepared.w_down, prepared.w_up
        self.partial = torch.empty(32 * R * config.global_split, dtype=torch.float32, device=self.device)
        self.output = torch.empty_like(prepared.output)
        x = torch.empty(self.rows * K, dtype=self.dtype, device=self.device)
        stream = torch.cuda.current_stream(self.device)
        if config.global_split > 1:
            self.up_config = replace(prepared.config, down_mode="partial", down_n=64, split_k=config.global_split)
            self.up_config.validate()
            _, up = control._launchers(self.rows, self.dtype, self.up_config)
            self.up = flyc.compile(up, x, self.w_up, self.partial, self.output.view(-1), stream)
        else:
            self.up = prepared.up
        self.down = flyc.compile(down_launcher(self.rows, config), x, self.w_down, self.partial, stream)
        self.dispatch = flyc.compile(
            pair_launcher(self.rows, config, self.up_config),
    ...
    def __call__(self, x):
        _check_input(self, x)
        self.dispatch(
            x.view(-1), self.w_down, self.w_up, self.partial, self.output.view(-1),
