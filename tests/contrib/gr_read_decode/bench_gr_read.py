...
if __package__:
    from .test_gr_read import assert_close, capture, make_inputs, parse_rows, reference
else:
    from test_gr_read import assert_close, capture, make_inputs, parse_rows, reference
...
def benchmark_rows(rows, pairs, args, emit):
    import torch
    if __package__:
        from .kernel import GRReadDecode, prepare_weights
    else:
        from kernel import GRReadDecode, prepare_weights
    ...
    with path.open("x") as out:
        ...
        with torch.inference_mode():
            ...
            for rows in args.rows:
                benchmark_rows(rows, pairs, args, emit)
        torch.cuda.synchronize()
        time.sleep(2)
        snap = hardware_snapshot()
