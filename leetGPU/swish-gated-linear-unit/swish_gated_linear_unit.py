...
import pytest
import pyhip
@pyhip.module("swiglu_kernel.cpp")
def swiglu_kernel(input,output,N): ...
...
def reference_impl(input: torch.Tensor, output: torch.Tensor, N: int):
    ...
    # 2. 计算 SwiGLU: SwiGLU(x, y) = SiLU(x) * y
    res = F.silu(x) * y
    output.copy_(res)
...
def test_sigmoid_linear_unit():
    ...
    output_tensor = torch.empty(N//2, device="cuda", dtype=torch.float32)
    ref_output_tensor = torch.empty(N//2, device="cuda", dtype=torch.float32)
    reference_impl(input_tensor,ref_output_tensor,N)

    threadsPerBlock = 256
    blocksPerGrid = (N//2 + threadsPerBlock - 1) // threadsPerBlock
   # breakpoint()
    kernel([blocksPerGrid],[threadsPerBlock],input_tensor.data_ptr(), output_tensor.data_ptr(), N//2)
